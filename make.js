...
async function shell(procName, argsArray = []) {
  ...
  if (!status.success) {
    throw new Error(
      `${procName} ${argsArray} exited with status ${status.code}`,
    );
  }
}
...
async function testDom() {
  ...
  for (const file of files) {
    ...
    if (served404) {
      console.log(
        `${file} failed: a background or content script requested a missing file.`,
      );
    }
...
desc(
  "Build a static version of command_listing.html, to be hosted on vimium.gihub.io",
);
...
desc(
  "Replaces manifest.json with a Firefox-compatible version, for development",
);
